...
int buse_main(const char* dev_file, const struct buse_operations *aop, void *userdata)
{
  ...
  if (nbd == -1) {
    fprintf(stderr, 
        ...
        "[error]: Is kernel module `nbd' is loaded and do you have permissions "
  ...
  assert(err != -1);
    ...
    fprintf(stdout, "Rescue file was successfully decrypted. %s is a block device containing the decrypted volume.\n", dev_file);
  ...
  if (!fork()) {
    ...
    if(ioctl(nbd, NBD_SET_SOCK, sk) == -1){
      fprintf(stderr, "[error]: ioctl(nbd, NBD_SET_SOCK, sk) failed.[%s]\n", strerror(errno));
    }
#if defined NBD_SET_FLAGS && defined NBD_FLAG_SEND_TRIM
    else if(ioctl(nbd, NBD_SET_FLAGS, NBD_FLAG_SEND_TRIM) == -1){
      fprintf(stderr, "[error]: ioctl(nbd, NBD_SET_FLAGS, NBD_FLAG_SEND_TRIM) failed.[%s]\n", strerror(errno));
    }
#endif
    else{
